...
第三方应用前端只负责把后端生成的 `audience`、`nonce`、scope 传给 Wallet；`appId` 仅作为可选展示上下文，不是钱包身份 presentation 的必需字段，也不应被当作 Node 应用主键。登录 session 可以返回 `issuerEndpoint`，`loginWithWalletIdentity` 会把它透传给 Wallet，用于过期邮箱/用户名凭证的 Node challenge 自动续签。登录是否成功由应用后端校验钱包身份 presentation 后决定。仅需地址时继续使用 `loginWithChallenge`。需要邮箱时，请求 `identity.email` scope，并以后端验证过的 `EmailCredential` 为准。
