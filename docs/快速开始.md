...
web3 应用按需求选择协议：只需地址登录使用 SIWE；需要用户名、邮箱或头像使用 Wallet Identity；需要资源权限使用 UCAN。不要把 Wallet Identity 当作所有钱包的强制登录方式。
...
- 若要走 UCAN，多后端授权链路要求可完成 SIWE 签名（Root），session 签名优先用钱包 UCAN RPC：
  - `wallet_ucan_session`（可选，钱包托管 UCAN session）
  - `wallet_ucan_sign`（可选，钱包托管 UCAN 签名）
...
需要已验证用户名、邮箱或头像时，使用独立的 Wallet Identity 登录：

```ts
import { loginWithWalletIdentity } from '@yeying-community/web3-bs';

const identity = await loginWithWalletIdentity({
  provider,
...
Wallet Identity 不要求先连接账户，也不自动执行 SIWE。完整流程见[钱包身份登录接入](./身份授权/钱包身份登录接入.md)。

没有 Wallet 插件时，使用 Wallet Identity authorization code + PKCE + Passkey 登录同一个钱包身份 DID，完整流程见[无插件身份登录接入](./身份授权/无插件身份登录接入.md)。
...
const login = await loginWithSiwe({
...
更详细的有效期说明见 [UCAN 有效期与续期](./身份授权/UCAN有效期与续期.md)。
...
#### 4.3.1 App 钱包支持 Wallet Identity

该路线要求 Provider 实现 `wallet_requestPermissions`、`wallet_identity` 和 `wallet_identity_presentation`。只支持标准 EIP-1193 的钱包应使用 `requestAccounts` 和 `loginWithSiwe`。

登录协议选择、页面顺序和后端校验要求见：[登录授权协议选择](./登录认证/登录授权协议选择.md)。
...
- 应用后端本地校验钱包身份 presentation；需要邮箱/用户名时校验 JWT-VC issuer/JWKS/有效期，并按风险等级使用本地 credential status 缓存或在线 Node 查询。持有有效本地凭证的正常登录不要求实时访问 Node
- `createWebDavClient`
...
const identityLogin = await loginWithWalletIdentity({
  provider,
  baseUrl: '/api/v1/public/auth',
  scopes: ['identity.basic', 'identity.wallet', 'identity.email', 'identity.avatar'],
});

console.log(identityLogin.did, identityLogin.walletAddress);
```
...
DApp 端用独立于钱包密码的"数据密码"对任意数据进行加解密，密文走 `v1:<suite>:...` 格式可粘贴存储。详见[加解密服务](./接口参考/加解密服务.md)。
...
- 插件钱包 + SIWE/JWT 登录：`getProvider` + `requestAccounts` + `loginWithSiwe` + `authFetch`
...
- App 钱包 + SIWE/JWT 登录：`requestAccounts` + `loginWithSiwe` + `authFetch`
