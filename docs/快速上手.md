...
- 应用后端校验钱包身份 presentation；需要邮箱/用户名时校验 JWT-VC issuer/JWKS/status
- `createWebDavClient`
...
const identityLogin = await loginWithWalletIdentity({
  ...
  createSession: () => fetch('/api/v1/public/auth/identity/login/session', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  }).then((res) => res.json()).then((json) => json.data),
  verifyPresentation: (presentation) => fetch('/api/v1/public/auth/identity/login/verify', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
