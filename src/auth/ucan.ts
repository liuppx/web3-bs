...
export type SiweUcanRootProofOptions = {
    type: 'siwe'
    provider?: Eip1193Provider
    address?: string
    domain?: string
...
export type UcanRootProofOptions = SiweUcanRootProofOptions

export type CreateUcanRootOptions = {
    proof: UcanRootProofOptions
...
type CreateSiweUcanRootOptions = Omit<CreateUcanRootOptions, 'proof'> & Omit<SiweUcanRootProofOptions, 'type'>
...
export function getUcanTokenTiming(token: string, options: { nowMs?: number } = {}): UcanTokenTiming {
    ...
    const issuedAt = typeof payloadWithIat?.iat === 'number' ? payloadWithIat.iat : null
...
export function isUcanTokenFresh(
    ...
    const timing =
        typeof tokenOrTiming === 'string' ? getUcanTokenTiming(tokenOrTiming, { nowMs: options.nowMs }) : tokenOrTiming
...
export async function getUcanSession(
    ...
    const walletProvider =
        provider || (typeof window !== 'undefined' ? await getProvider({ preferYeYing: true }) : null)
    if (walletProvider) {
        try {
...
export async function createUcanSession(options: CreateUcanSessionOptions = {}): Promise<UcanSessionKey> {
    const sessionId = parseSessionId(options)
    const record = await readSessionRecord(sessionId)
    const provider =
        options.provider || (typeof window !== 'undefined' ? await getProvider({ preferYeYing: true }) : null)
    if (provider) {
        try {
...
export async function storeUcanRoot(root: UcanRootProof, id: string = DEFAULT_SESSION_ID): Promise<void> {
    const record = await readSessionRecord(id)
...
export async function resolveUcanAuthorization(
    ...
    const root =
        options.root === undefined ? await getStoredUcanRoot(options.sessionId || DEFAULT_SESSION_ID) : options.root
    const nowMs = options.nowMs ?? Date.now()
    ...
    if (options.expectedCapabilities && !capsEqual(root.cap, options.expectedCapabilities)) {
        return {
            status: 'unauthorized',
...
export async function getOrCreateUcanRoot(options: CreateUcanRootOptions): Promise<UcanRootProof> {
    const provider = options.proof.provider || (await requireProvider())
    const session = options.session || (await createUcanSession({ id: options.sessionId, provider }))
    const address = await resolveAddress(provider, options.proof.address)
    const expectedIssuer = `did:pkh:eth:${address.toLowerCase()}`
    const nowMs = Date.now()
    const stored = await getStoredUcanRoot(session.id)
    if (
        stored &&
        stored.type === options.proof.type &&
        stored.iss === expectedIssuer &&
    ...
    return await createRootUcan({ ...options, session, proof: { ...options.proof, provider, address } })
}
...
async function signWithProvider(provider: Eip1193Provider, address: string, message: string): Promise<string> {
...
async function createSiweUcanRoot(options: CreateSiweUcanRootOptions): Promise<UcanRootProof> {
...
export async function createRootUcan(options: CreateUcanRootOptions): Promise<UcanRootProof> {
    switch (options.proof.type) {
        case 'siwe':
            return createSiweUcanRoot({
                session: options.session,
                sessionId: options.sessionId,
                capabilities: options.capabilities,
                expiresInMs: options.expiresInMs,
                notBeforeMs: options.notBeforeMs,
                provider: options.proof.provider,
                address: options.proof.address,
                domain: options.proof.domain,
                uri: options.proof.uri,
                chainId: options.proof.chainId,
                statement: options.proof.statement,
                nonce: options.proof.nonce
            })
        default:
            throw new Error('UCAN_ROOT_PROOF_UNSUPPORTED')
    }
}
...
async function resolveProofs(options: CreateUcanTokenOptions, issuer?: UcanSessionKey): Promise<UcanProof[]> {
...
export async function getOrCreateInvocationUcan(options: GetOrCreateInvocationUcanOptions): Promise<string> {
    if (
        options.ucan &&
