...
import { normalizeUcanCapabilities, normalizeUcanCapability, type UcanCapability } from './ucan'
...
export async function getCentralIssuerInfo(options: CentralAuthBaseOptions = {}): Promise<CentralIssuerInfo> {
...
export async function createCentralSession(options: CreateCentralSessionOptions): Promise<CentralSessionResult> {
...
export async function issueCentralUcan(options: IssueCentralUcanOptions = {}): Promise<CentralUcanIssueResult> {
    const sessionToken = options.sessionToken || getCentralSessionToken(options)
    if (!sessionToken) {
        throw new Error('Missing central session token')
    }

    const normalizedCapabilities = options.capabilities ? normalizeUcanCapabilities(options.capabilities) : undefined
    const fetcher = resolveFetcher(options)
    const credentials = resolveCredentials(options)
